...
import com.shop.shop_java.entity.StoreProductRule;
import com.shop.shop_java.service.StoreProductRuleService;
...
public class StoreProductRuleController {
    ...
    @Autowired
    private StoreProductRuleService productRuleService;
    ...
    public Map<String, Object> getList(
        ...
        LambdaQueryWrapper<StoreProductRule> wrapper = new LambdaQueryWrapper<>();
        if (keyword != null && !keyword.isEmpty()) {
            wrapper.like(StoreProductRule::getRuleName, keyword);
        }
        wrapper.orderByDesc(StoreProductRule::getId);

        Page<StoreProductRule> pageParam = new Page<>(page, limit);
        Page<StoreProductRule> resultPage = productRuleService.page(pageParam, wrapper);
    ...
    @PostMapping("/save")
    public Map<String, Object> save(@RequestBody StoreProductRule rule) {
        if (rule.getId() == null) {
            productRuleService.save(rule);
        } else {
            productRuleService.updateById(rule);
        }
        Map<String, Object> result = new HashMap<>();
        result.put("code", 200);
        result.put("msg", "保存成功");
    ...
    public Map<String, Object> delete(@RequestBody Map<String, Object> params) {
        ...
        if (idObj != null) {
            ...
            productRuleService.removeById(id);
        }
        Map<String, Object> result = new HashMap<>();
        result.put("code", 200);
        result.put("msg", "删除成功");
